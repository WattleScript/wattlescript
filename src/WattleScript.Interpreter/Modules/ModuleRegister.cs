...
namespace WattleScript.Interpreter
{
	...
	public static class ModuleRegister
	{
		...
		public static Table RegisterModuleType(this Table gtable, Type t)
		{
			...
			{
				if (mi.GetCustomAttributes(typeof(WattleScriptModuleMethodAttribute), false).ToArray().Length > 0)
				{
					WattleScriptModuleMethodAttribute attr = (WattleScriptModuleMethodAttribute)mi.GetCustomAttributes(typeof(WattleScriptModuleMethodAttribute), false).First();
			...
			foreach (FieldInfo fi in t.GetAllFields().Where(_mi => _mi.IsStatic && _mi.GetCustomAttributes(typeof(WattleScriptModuleMethodAttribute), false).ToArray().Length > 0))
			{
				WattleScriptModuleMethodAttribute attr = (WattleScriptModuleMethodAttribute)fi.GetCustomAttributes(typeof(WattleScriptModuleMethodAttribute), false).First();
				string name = (!string.IsNullOrEmpty(attr.Name)) ? attr.Name : fi.Name;

				RegisterScriptField(fi, null, table, t, name);
			}

			foreach (FieldInfo fi in t.GetAllFields().Where(_mi => _mi.IsStatic && _mi.GetCustomAttributes(typeof(WattleScriptModuleConstantAttribute), false).ToArray().Length > 0))
			{
				WattleScriptModuleConstantAttribute attr = (WattleScriptModuleConstantAttribute)fi.GetCustomAttributes(typeof(WattleScriptModuleConstantAttribute), false).First();
				string name = (!string.IsNullOrEmpty(attr.Name)) ? attr.Name : fi.Name;
		...
		{
			WattleScriptModuleAttribute attr = (WattleScriptModuleAttribute)(t.GetCustomAttributes(typeof(WattleScriptModuleAttribute), false).First());
