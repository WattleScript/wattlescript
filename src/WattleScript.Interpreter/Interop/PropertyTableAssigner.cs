...
namespace WattleScript.Interpreter.Interop
{
	...
	/// Properties must be decorated with the <see cref="WattleScriptPropertyAttribute"/>.
	...
	public class PropertyTableAssigner : IPropertyTableAssigner
	{
		...
		public PropertyTableAssigner(Type type, params string[] expectedMissingProperties)
		{
			...
			{
				foreach (WattleScriptPropertyAttribute attr in pi.GetCustomAttributes(true).OfType<WattleScriptPropertyAttribute>())
				{
					string name = attr.Name ?? pi.Name;

					if (m_PropertyMap.ContainsKey(name))
					{
						throw new ArgumentException(string.Format("Type {0} has two definitions for WattleScript property {1}", m_Type.FullName, name));
					}
					else
					{
