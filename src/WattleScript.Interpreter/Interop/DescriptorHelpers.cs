...
namespace WattleScript.Interpreter.Interop
{
	...
	public static class DescriptorHelpers
	{
		...
		/// <see cref="WattleScriptVisibleAttribute" /> or a <see cref="WattleScriptHiddenAttribute" />  is changing visibility of a member
		/// to scripts.
		/// </summary>
		...
		/// </returns>
		/// <exception cref="System.InvalidOperationException">If both WattleScriptHiddenAttribute and WattleScriptVisibleAttribute are specified and they convey different messages.</exception>
		public static bool? GetVisibilityFromAttributes(this MemberInfo mi)
		{
			if (mi == null)
				return false;

			WattleScriptVisibleAttribute va = mi.GetCustomAttributes(true).OfType<WattleScriptVisibleAttribute>().SingleOrDefault();
			WattleScriptHiddenAttribute ha = mi.GetCustomAttributes(true).OfType<WattleScriptHiddenAttribute>().SingleOrDefault();

			if (va != null && ha != null && va.Visible)
				throw new InvalidOperationException(string.Format("A member ('{0}') can't have discording WattleScriptHiddenAttribute and WattleScriptVisibleAttribute.", mi.Name));
			else if (ha != null)
				return false;
			else if (va != null)
				return va.Visible;
		...
		/// <see cref="WattleScriptUserDataMetamethodAttribute" /> .
		/// </summary>
		/// <param name="mi">The mi.</param>
		/// <returns></returns>
		public static List<string> GetMetaNamesFromAttributes(this MethodInfo mi)
		{
			return mi.GetCustomAttributes(typeof(WattleScriptUserDataMetamethodAttribute), true)
				.OfType<WattleScriptUserDataMetamethodAttribute>()
				.Select(a => a.Name)
				.ToList();
		}
