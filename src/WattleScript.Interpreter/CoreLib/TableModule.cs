...
namespace WattleScript.Interpreter.CoreLib
{
	...
	public class TableModule
	{
		...
		public static DynValue concat(ScriptExecutionContext executionContext, CallbackArguments args)
		{
			...
			int start = vstart.IsNilOrNan() ? executionContext.OwnerScript.Options.IndexTablesFrom : vstart.CastToInt() ?? 0;
			int end = vend.IsNilOrNan() ? GetTableLength(executionContext, vlist) : vend.CastToInt() ?? 0;
