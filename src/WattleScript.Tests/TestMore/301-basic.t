...
-- WattleScript note : THIS SUITE WAS HEAVILY CUT FOR NOT APPLIABLE TESTS
...
error_like(function () t = {}; rawset(t, nil, 42) end,
           "^[^:]+:%d+: table index is nil",   -- changed this for WattleScript, but we stay as it is!
...
is(type(nil), 'nil')
-- is(type(io.stdin), 'userdata') -- no stdin in WattleScript so far 
...
--[[ Doesn't throw an error on Lua 5.2.4
...
--[[ WattleScript : this is intentional - we support pack and unpack outside the table namespace (or whatever they are) 
